...
int main(int argc, char *argv[])
{
  ...
  if (pin_init_user(dev,LPT1) < 0)
    exit(0);

  pin_output_mode(dev,LP_DATA_PINS | LP_SWITCHABLE_PINS);
  ...
  while (1) {
    ...
    set_pin(dev,LP_PIN[pin_sequence[i]] | prev);
    printf("Hit return...\n");
    fgets(buf, 5, stdin);
    printf("clearing pin %d\n", pin_sequence[i]);
    clear_pin(dev,LP_PIN[pin_sequence[i]] | prev);
