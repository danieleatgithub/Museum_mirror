...
static char devname_default[] = "ppdrv";
...
int insert_module(void)
{
  ...
  printk(KERN_INFO "parapindriver: dev %s assigned major: %d\n", devname, major);
...
{
  int ret;
  unsigned int minor = iminor(inodePtr);
  ...
  /* Initialize kparapin */
  ret = pin_init_kernel(minor, (void *)NULL);

  printk(KERN_INFO "parapindriver: pin_init_kernel minor=%d ret=%d\n", minor,ret);
